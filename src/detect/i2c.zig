...
const SYS_I2C = "/sys/bus/i2c/devices";
...
pub fn detect(allocator: std.mem.Allocator) ![]const output.I2cDevice {
    var dir = std.fs.openDirAbsolute(SYS_I2C, .{ .iterate = true }) catch return &.{};
    defer dir.close();

    var list = std.ArrayList(output.I2cDevice).empty;
    ...
    while (iter.next() catch null) |entry| {
        ...
        const base_path = std.fmt.bufPrint(&base_buf, "{s}/{s}", .{ SYS_I2C, name }) catch continue;
